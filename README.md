...
Aplicação web: interface, App Router, persistência em `localStorage`, tema, sons, notificações do navegador, PWA e o `PomodoroContext` (React). Consome o domínio via `@destravai/core`. O fluxo “Travei” autentica com e-mail/senha ou Google, passa pelo proxy `POST /api/agents/unlock-task/runs` e só aplica o plano com confirmação explícita. Detalhes em `apps/web/README.md`.
