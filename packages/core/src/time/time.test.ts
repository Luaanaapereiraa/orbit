...
import { formatClock, getElapsedSeconds } from './index'
...
describe('getElapsedSeconds', () => {
  ...
  it('counts wall-clock time since start', () => {
    ...
    expect(getElapsedSeconds({ startDate: start, pausedMs: 0 }, now)).toBe(90)
  })
  ...
  it('freezes elapsed time while currently paused', () => {
    ...
    expect(
      getElapsedSeconds({ startDate: start, pausedMs: 0, pausedAt }, now),
    ).toBe(40)
  })
})
